...
ssize_t count = read(filedes[0], buffer, sizeof(buffer));
   ...
   // reading from childs stdout
	...
	memset(MyShell_output,0,sizeof(MyShell_output));
    memcpy(MyShell_output, buffer, MAX_STDOUT);
    printf("MyShell output written successfully after child completed in the background\n");
	}
}
...
if(cpid == 0) {
 ...
 if (execvp(args[0], args) == -1){
  ...
  if (count == -1) {
    ...
    memset(MyShell_output,0,sizeof(MyShell_output));
    memcpy(MyShell_output, buffer, MAX_STDOUT);
    printf("Output has been succesfully written by the parent\n");
...
void start()
{
   static void (*op_func[])() = {OpPut, OpGet, OpGet, OpExit,OpShell} ; // OpShell added
...
void OpShell()
{
   ...
   args = tokenize_input(t,SHELL_LINE_DELIM); 
   out.status = shell_execute(args); 
   free(t);
   free(args);
