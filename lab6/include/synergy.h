...
//#define MAX_CMD 100
#define MAX_STDOUT 100000   // needed increased size for reading from large files
#define MAX_CWD 1000
#define MAX_USERNAME 30

extern int background;
extern char MyShell_output[MAX_STDOUT];
...
#define TSH_OP_SHELL              405 // for lab6
...
typedef struct{ 
    ...
    sng_int16 error;
	size_t length ;
    char out_buffer[MAX_STDOUT]; 
	char username[MAX_USERNAME]; 
    char cwd_loc[MAX_CWD];
...
typedef struct {
  ...
  sng_int16 priority ;
  sng_int32 host ;
  sng_int16 port ; // which port we replay too
  sng_int32 length ;
  int proc_id;
} tsh_put_it ;
